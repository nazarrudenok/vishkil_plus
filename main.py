...
def start(message):
    if message.from_user.username == None:
        ...
        if is_exists == ():
            ...
            msg = bot.send_message(message.chat.id, 'Вітаю, друже! Ти вже пройшов другий етап відбору на «Вишкіл Військового Лідерства 2026». Далі тобі необхідно створити особистий кабінет на порталі Вишкіл+\nНапиши своє прізвище та ім\'я')
...
- Так, звʼязок є.
<b>4. Що брати їсти?</b>
- Забезпечується повне харчування впровож всіх 4 днів - 3 рази на день (сніданок, обід, вечеря). Додатково з собою можна взяти ще щось.
"""
...
Всім учасникам необхідно мати плитоноску/бронежилет з підсумками(обов'язково аптечка з тренувальним турнікетом ), РПС та наколінники за бажанням, щоб отримати це спорядження можна звернутись до своїх ЦНПВ, у випадку, якщо з таким майном будуть проблеми, прохання написати в чаті.
"""
